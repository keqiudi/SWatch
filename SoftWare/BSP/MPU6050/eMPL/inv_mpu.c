...
//
void mget_ms(unsigned long *time) //�����պ������У��ǵ÷Ž�ͷ�ļ��������滹�õõ�
{
...
static signed char gyro_orientation[9] = { 1, 0, 0,  ////陀螺仪方向设置，方向矩阵
                                           0, 1, 0,
                                           0, 0, 1};
...
uint8_t run_self_test(void) 
{
    ...
    if (result == 0x3) {   // 从0x7修改为0x3, 我们只需要加速度计和陀螺仪的自检
...
{ 
    ...
    if(mpu_init() == 0)
    {   
        res = mpu_lp_accel_mode(20); // 进入低功耗加速度计模式，设置频率为20Hz
        if(res) return -1;
        // res = mpu_set_sensors(INV_XYZ_ACCEL); // 启用加速度计的三轴数据
        // if(res) return 1;
        res = mpu_configure_fifo(INV_XYZ_ACCEL); // 配置FIFO仅存储加速度计数据
        if(res) return 2;
        res = mpu_set_sample_rate(DEFAULT_MPU_HZ); // 设置采样率为默认值（如100Hz）
        if(res) return 3;
        res = dmp_load_motion_driver_firmware(); // 加载DMP运动驱动固件
        if(res) return 4;
        // res = dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation)); // 设置陀螺仪方向
        // if(res) return 5;
        res = dmp_enable_feature(DMP_FEATURE_TAP| DMP_FEATURE_SEND_RAW_ACCEL| DMP_FEATURE_PEDOMETER); // 启用DMP特性：敲击检测、原始加速度数据、计步器
        if(res) return 6;
        res = dmp_set_fifo_rate(DEFAULT_MPU_HZ); // 设置DMP输出速率(最大不超过200Hz)
        if(res) return 7;
        //res = run_self_test(); // 运行自检程序（校准传感器）
        //if(res) return 8;
        res = mpu_set_dmp_state(1); // 启用DMP功能
        if(res) return 9;
    }
    else
    {
